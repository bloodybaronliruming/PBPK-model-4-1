...
def featurize(smiles, descriptor_names=None, progress=True):
    """只计算确定性结构特征；插补、缩放都在 fit_model 的训练范围内。"""
    names = descriptor_names if descriptor_names is not None else [name for name,_ in Descriptors._descList]
    ...
    cache = {}
    for smi in tqdm(unique, desc='ECFP4 / RDKit 2D', disable=not progress):
        ...
        cache[smi] = np.concatenate([bits,np.asarray(values,dtype=np.float32)])
...
def make_estimator(algorithm, params, seed, threads, device, epochs, batch_size):
    ...
    if algorithm=='xgboost':
        ...
        return XGBRegressor(objective='reg:squarederror',tree_method='hist',random_state=seed,n_jobs=threads,**params)
    if algorithm=='lightgbm':
        from lightgbm import LGBMRegressor
        return LGBMRegressor(random_state=seed,n_jobs=threads,verbosity=-1,**params)
    if algorithm=='catboost':
        from catboost import CatBoostRegressor
        return CatBoostRegressor(random_seed=seed,thread_count=threads,verbose=False,allow_writing_files=False,**params)
...
def fit_model(frame, X, indices, algorithm, parameters, spec, names, args):
    ...
    return ModelBundle(algorithm,estimator,prep,target,spec,names,sorted(training.molecule_id.unique()),
                       sorted(training.scaffold_group.unique()),parameters,rdBase.rdkitVersion)
...
def train_endpoint(endpoint, default_system):
    ...
    if args.device=='cuda' and set(algorithms)&{'resmlp','dmpnn'}:
        import torch
    ...
    logging.info('task=%s；有效训练记录=%d，分子=%d，folds=%s；算法=%s',task,len(idx),frame.iloc[idx].molecule_id.nunique(),folds,algorithms)
    if args.check_only:
        logging.info('前序、标签、单位、划分、算法依赖及资源请求检查通过；尚未训练。')
        return
    tuning, summary = [], {}
    with threadpool_limits(limits=args.threads), stage_output(output) as out:
        X,names = featurize(frame.smiles.tolist())
        dump_json(out/'feature_schema.json',dict(fingerprint='Morgan radius=2 nbits=2048',descriptor_names=names,rdkit=rdBase.rdkitVersion,
                   feature_order=[f'ECFP4_{i}' for i in range(2048)]+names,fit_scope='deterministic_structure_only'))
        ...
        finish_stage(out,'stl_training',inputs={'datasets_complete_sha256':sha256(datasets/'complete.json'),
                     ...
                     test_evaluated=args.evaluate_test,algorithms=algorithms,seed=args.seed,partial=False)
